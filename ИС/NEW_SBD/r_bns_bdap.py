...
def get_source_conn() -> any:
    ...
    """
    try:
        conn_secrets = Connection.get_connection_from_secrets(conn_id="mne_ks_orcl_sdu")

        conn = BaseHook.get_connection("GBDFL_Oracle_17_FL")
        extra_params = conn.extra_dejson

        dsn_data = None

        if extra_params.get("sid") is not None:
            dsn_data = cx_Oracle.makedsn(host=conn_secrets.host, port=conn_secrets.port, extra_params.get("sid"))
        elif extra_params.get("service_name") is not None:
            dsn_data = cx_Oracle.makedsn(host=conn_secrets.host, port=conn_secrets.port, service_name=extra_params.get("service_name"))
...
def get_dag_configs(key):
    configs = {
        'dag_id': "r_bns_bdap",
        'owner': "Bakhtiyar",
        'start_date': datetime.datetime.strptime('2/8/2023', '%d/%m/%Y'),
        'schedule_interval': "0 0 * * *",
        'tags': ['Oracle', 'bns', 'reglament'],
    }
...
#   Reglament update, truncate load case
    ...
    METADATA = [
        {
            "source_schema": "BDAP_NEW",
            "target_schema": "R_BNS_BDAP",
            "tables": [
                {
                    "source_table": "BDAP_COMMIT_PACKAGE",
                    "hash_column_indexes": [],
                    "n_columns": 19,
                    "sql": """SELECT PACKAGE_ID, NAME, RECEIVE_DATE, INITIATOR, MESSAGE, DATA, FORM_ID, REPORTING_PERIOD, DATA_TYPE_ID, STATUS_ID, VERSION, PAGE_TOTAL, PAGE_LOAD, REC_COUNT, SUB_SYSTEM_ID, TIME_ISO, PRIORITY FROM BDAP_NEW.BDAP_COMMIT_PACKAGE"""
                # },
                # {
                #     "source_table": "BDAP_CUBE2",
                #     "hash_column_indexes": [],
                #     "n_columns": 11,
                #     "sql": None
                # },
                # {
                #     "source_table": "BDAP_FACT_MVIEW",
                #     "hash_column_indexes": [],
                #     "n_columns": 13,
                #     "sql": None
                # }
