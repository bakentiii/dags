...
def get_source_conn() -> any:
    ...
    """
    try:
        conn_secrets = Connection.get_connection_from_secrets(conn_id="mu_enotariat")
...
def get_dag_configs(key):
    configs = {
        'dag_id': "r_mu_enotariat",
        'owner': "Bakhtiyar",
        'start_date': datetime.datetime.strptime('', '%d/%m/%Y'),
        'schedule_interval': "0 0 * * *",
        'tags': ['mu', 'enotariat', 'reglament'],
    }
    ...
    METADATA = [
        {
            ...
            "target_schema": "MU_ENOTARIAT",
            "tables": [
                {
                    "source_table": "RNOTARIALOPERATION",
                    "hash_column_indexes": [
                        5
                    ],
                    "n_columns": 12,
