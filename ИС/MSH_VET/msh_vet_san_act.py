...
def get_dag_configs(key):
    configs = {
        'dag_id': "r_msh_vet",
        'owner': "Bakhtiyar",
        'start_date': datetime.datetime.strptime('3/8/2023', '%d/%m/%Y'),
        'schedule_interval': "0 0 * * *",
        'tags': ['mysql/mariadb', 'msh', 'reglament'],
    }
...
#   Reglament update, truncate load case
    ...
    METADATA = [
        {
            "source_schema": "msh_prod",
            "target_schema": "MSH_VET",
            "tables": [
                {
                    "source_table": "msh_vet_san_act",
                    "hash_column_indexes": [],
                    "n_columns": 21,
                    "sql": """select * from msh_vet_san_act"""
                }
                ]
        }
    ]
